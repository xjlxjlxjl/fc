<template>
  <div>
    <div id="addSplit" class="modal fade" role="dialog">
      <div class="modal-dialog" role="document" style="width: 1280px;max-width: 100%;">
        <div class="modal-content">
          <div class="modal-body">
            <p class="table-label">原料</p>
            <el-table :data="material" border stripe>
                ...
                <template slot-scope="{ row }">
                  <el-input v-model="row.material_number" size="mini">
                    <el-button
                      ...
                      @click="type = 1;getMater(true)"
                    ></el-button>
                  </el-input>
                </template>
              </el-table-column>
              <el-table-column prop="material_specification" label="料品规格">
                <template slot-scope="{ row }">
                  <el-input v-model="row.material_specification" size="mini">
                    <el-button
                      size="mini"
                      slot="suffix"
                      icon="el-icon-arrow-down"
            ...
            <p class="table-label">拆分料</p>
            <el-table :data="split_material" border stripe>
              <el-table-column width="50" label="序号">
                <template slot-scope="{ $index }">{{ $index + 1 }}</template>
              </el-table-column>
              <el-table-column prop="material_number" label="料品编码">
              ...
              <el-table-column prop="percentage" label="百分比">
                <template slot-scope="{ row }">
                  <el-input v-model="row.percentage" size="mini"></el-input>
                </template>
              </el-table-column>
              <el-table-column prop="item_unit" label="单位"></el-table-column>
              <el-table-column prop="respository.name" label="仓库"></el-table-column>
            ...
            <p class="table-label">余料</p>
            <el-table :data="expected" border stripe>
              <el-table-column width="50" label="序号">
                <template slot-scope="{ $index }">{{ $index + 1 }}</template>
              </el-table-column>
              <el-table-column prop="material_number" label="料品编码">
  ...
  methods: {
    ...
    addMater() {
      const active = new Map([
        [1, () => {
          this.material.pop();
          this.material.push(this.mater.selection);
        }],
        [2, () => {
          this.split_material.pop();
          this.split_material.push(this.mater.selection);
        }],
        [3, () => {
          this.expected.pop();
          this.expected.push(this.mater.selection);
        }],
        ['default', () => this.$message({ message: '未选择料品类型', type: 'error' })]
      ]);
      active.get(this.type || 'default').call(this);
      $("#split .materList").modal("hide");
    },
    add() {
      let arr = [];
      console.log(this.material)
      for (let e of this.material)
        arr.push({
          material_id: e.id,
          count: e.count,
          percentage: '100',
      ...
      for (let e of this.split_material)
        arr.push({
          material_id: e.id,
          count: e.count,
      ...
      this
        .$post(`repositories/split_order/create`, {
          materials: arr
        })
        .then(response => {
          if (response.status != 200) return false;
...
<style lang="less">
@tableLbaleBgc: #cccccc;
#addSplit {
  .table-label {
    padding: 6px 12px;
    background-color: @tableLbaleBgc;
    font-weight: 900;
    font-size: 20px;
    margin-bottom: 0px;
  }
