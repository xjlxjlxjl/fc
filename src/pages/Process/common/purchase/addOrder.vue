      ...
      <div class="modal-dialog modal-lg" style="width: 100%;max-width: 1280px;" role="document">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel">新建采购订单</h4>
          </div>
            ...
            <el-form :model="form" size="mini" ref="form" label-width="80px">
              ...
              <el-form-item class="table">
                <el-table :data="form.items" size="mini" border style="width: 100%">
                  ...
                  <el-table-column prop="purchaseApply" label="关联采购计划" width="120px">
                    <template slot-scope="{$index, row}">
                      <el-input v-model="row.purchaseApply" placeholder="关联采购计划">
                        <el-button
                          slot="append"
                  ...
                  <el-table-column prop="saleOrder" label="关联销售订单" width="120px">
                    <template slot-scope="{$index, row}">
                  ...
                  <el-table-column prop="code" label="料品编码" width="120px">
                    <template slot-scope="{$index, row}">
                  ...
                  <el-table-column prop="supplier" label="供应商">
                    <template slot-scope="{$index, row}">
                      <el-input v-model="row.supplier" placeholder="供应商" width="120px">
                        <el-button slot="append" icon="el-icon-arrow-down" @click="getSupplier();index = $index"></el-button>
                      </el-input>
                    </template>
                  </el-table-column>
                  <el-table-column prop="price" label="单价">
                    <template slot-scope="{$index, row}">
                      <el-input v-model="row.price" placeholder="单价"></el-input>
                    </template>
                  </el-table-column>
                  <el-table-column prop="date" label="交期">
                    <template slot-scope="{$index, row}">
                      <el-input v-model="row.date" placeholder="交期"></el-input>
                    </template>
                  </el-table-column>
                </el-table>
              </el-form-item>
      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          <el-table
            ...
            style="width: 100%"
            height="500"
            @selection-change="supplierChange"
          >
            <el-table-column type="selection"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getPurchaseApply() {
      ...
      that
        ...
        .then(response => {
          ...
          $("#purchaseOrder .applyList").modal("show");
        })
    ...
    getSaleOrder() {
      ...
      that
        ...
        .then(response => {
          ...
          that.order.data = arr;
          $("#purchaseOrder .orderList").modal("show");
        })
        .catch(err => loading.close());
    },
    addApply() {
      ...
      if (this.apply.selection.length > 1 && !this.request_id) {
        ...
        that
          ...
          .then(response => {
            if (response.status != 200) return false;
            that.request_id = response.data.order_id;
            that.addItem("apply", $("#purchaseOrder .applyList"));
            params.id = response.data.id;
            that.form.items.unshift(params);
          })
          .catch(err => console.error(err));
      } else this.addItem("apply", $("#purchaseOrder .applyList"));
    },
    addOrder() {
    ...
    getMater() {
      ...
      that
        ...
        .then(response => {
          ...
          $("#purchaseOrder .materList").modal("show");
        })
        .catch(err => loading.close());
    },
    materChange(val) {
      ...
      if (this.mater.selection.length > 1 && !this.request_id) {
        ...
        that
          ...
          .then(response => {
            if (response.status != 200) return false;
            that.request_id = response.data.order_id;
            that.addItem("mater", $("#purchaseOrder .materList"));
            params.id = response.data.id;
            that.form.items.unshift(params);
          })
          .catch(err => console.error(err));
      } else this.addItem("mater", $("#purchaseOrder .materList"));
    },
    ...
    addSupplier() {
      if(this.supplier.selection.length > 0)
        this.form.items[this.index].supplier = this.supplier.selection[0].abbreviation
    ...
    onSubmit() {
      ...
      that
        .$post(`procurement/order/edit/${that.request_id}`, {
        ...
        .then(response => {
          if (response.status != 200) return false;
          that.refresh($("#purchaseOrder #table"));
          $("#addOrder").modal("hide");
      ...
      handler(val, oldVal) {
        ...
        if (val.items.length - oldVal.items.length > 1) return false;
          ...
          if (addRows) {
            ...
            that
              .$post(`procurement/order/item/create`, {
                order_id: that.request_id || "",
                code: lastRow.code || "",
            ...
            if (id)
              that.$post(`procurement/order/item/edit/${id}`, {
                order_id: that.request_id || "",
                code: lastRow.code || "",
                material_id: lastRow.material_id,
                name: lastRow.name || "",
