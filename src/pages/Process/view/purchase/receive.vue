          ...
          <div class="modal-body">
            ...
            >
              ...
              >
                ...
                <div
                  style="width: 100%;height: 40px;font-size: 13px;line-height: 40px;text-align: center;border-top: 1px solid;"
                >{{ modalData.outsourcing_number }}</div>
                <div
                  style="width: 100%;height: 40px;font-size: 13px;line-height: 40px;text-align: center;border-top: 1px solid;"
...
export default {
  ...
  methods: {
    ...
    init() {
      let that = this,
        ...
        data = {
          toolbar: "#purchasReceive #toolbar",
      ...
      $("#purchasReceive #table").bootstrapTable(data);
    },
    printing() {
  ...
  mounted() {
    const that = this;
    this.init();
    $("#purchasReceive").on("change", ".closeCase", function() {
      let self = $(this),
        id = self.attr("index"),
        data = that.getRow($("#purchasReceive #table"), id);
      ...
      that
        ...
        .then(response => {
          if (response.status != 200) return false;
          that.editRow($("#purchasReceive #table"), id, data);
        })
        .catch(err => console.error(err));
    });
  }
