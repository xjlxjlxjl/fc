      ...
      let that = this,
        columns = [
          ...
          },
          {
            ...
            title: "二维码",
            formatter(value, row, index) {
              setTimeout(() => QRCode.toString(value, (err, string) => (document.getElementById(`purchaseApplyQrcode${row.id}`).innerHTML = string)), 500);
              return `<div id="purchaseApplyQrcode${row.id}" class="img" style="width: 50px;height: 50px;margin: auto;"></div>`;
            },
            events: {
              "click .img": function(e, value, row, index) {
                that.modalData = row;
                that.modalData.company = that.user.user.current_company;
                QRCode.toString(value, (err, string) => (document.getElementById("purchaseApplyPrintImg").innerHTML = string));
