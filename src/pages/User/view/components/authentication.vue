  ...
  <div id="authentication">
    <el-form :model="formData">
      <el-form-item label="姓名">
        <el-input v-model="formData.last_name" :disabled="isDisabled"></el-input>
      </el-form-item>
      <el-form-item label="身份证">
        <el-input v-model="formData.id_card" :disabled="isDisabled"></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    beforeAvatarUploadPositive(file) {
      ...
      that
        .$upload("members/user/file-upload", form)
        .then(response => {
          loading.close();
    ...
    save() {
      ...
      let that = this;
      that
        .$post("members/user/real-verification", that.formData)
        .then(response => {
          loading.close();
          if (response.status != 200) return false;
          that.$message({ message: response.message, type: "success" });
