<template>
  <div id="apply">
    <div style="width: 600px;height: 840px;border: 1px solid rgba(121, 121, 121, 1);padding: 30px;">
      <p style="text-align: center;font-size: 18px;">产品报价单</p>
      <div
        style="display: flex;justify-content: space-between;border-bottom: 1px solid rgb(121, 121, 121);font-size: 10px;padding: 20px 0;"
      >
        <div>
          <p>公司：{{ data.company.name }}</p>
          <p>地址：{{ data.company.address }}</p>
          <p>
            <span style="margin-right: 25px;">电话：{{ data.company.phone }}</span>
            <span>传真：{{ data.company.fax }}</span>
          </p>
          <p>
            <span style="margin-right: 25px;">网址：{{ data.company.website }}</span>
            <span>邮箱：{{ data.company.email }}</span>
          </p>
        </div>
        <div id="QrCode" style="width: 80px;height: 80px;"></div>
      </div>
      <div style="margin-top: 15px;font-size: 10px;line-height: 2;">
        <p style="display: flex;justify-content: space-between;">
          <span style="width: 50%;">销售订单号：{{ data.number }}</span>
          <span style="width: 50%;">报价人：{{ data.created_by }}</span>
          <span style="width: 50%;">手机号：{{ data.created_mobile }}</span>
        </p>
        <p style="display: flex;justify-content: space-between;">
          <span style="width: 50%;">报价日期：{{ data.created_at }}</span>
          <span style="width: 50%;">报价有效期至：{{ data.validity }}</span>
          <span style="width: 50%;">币别：{{ data.currency }}</span>
        </p>
        <p style="display: flex;justify-content: space-between;">
          <span style="width: 50%;">客户名：{{ data.customer_name }}</span>
          <span style="width: 50%;">联系人：{{ data.linkman }}</span>
          <span style="width: 50%;">手机号：{{ data.mobile }}</span>
        </p>
        <p style="display: flex;justify-content: space-between;">
          <span style="width: 50%;">公司地址：{{ data.address }}</span>
        </p>
        <table
          style="margin-top: 10px;width: 100%;border: 1px solid rgb(215, 215, 215);text-align: center;"
        >
          <tr>
            <td style="border-right: 1px solid rgb(215, 215, 215)">序号</td>
            <td style="border-right: 1px solid rgb(215, 215, 215)">产品名称</td>
            <td style="border-right: 1px solid rgb(215, 215, 215)">产品规格型号</td>
            <td style="border-right: 1px solid rgb(215, 215, 215)">单位</td>
            <td style="border-right: 1px solid rgb(215, 215, 215)">数量</td>
            <td style="border-right: 1px solid rgb(215, 215, 215)">单价</td>
            <td>备注</td>
          </tr>
          <tr v-for="(item,index) in data.items" :key="index">
            <td
              style="border-right: 1px solid rgb(215, 215, 215);border-top: 1px solid rgb(215, 215, 215);"
            >{{ index + 1 }}</td>
            <td
              style="border-right: 1px solid rgb(215, 215, 215);border-top: 1px solid rgb(215, 215, 215);"
            >{{ item.name }}</td>
            <td
              style="border-right: 1px solid rgb(215, 215, 215);border-top: 1px solid rgb(215, 215, 215);"
            >{{ item.model }}</td>
            <td
              style="border-right: 1px solid rgb(215, 215, 215);border-top: 1px solid rgb(215, 215, 215);"
            >{{ item.unit }}</td>
            <td
              style="border-right: 1px solid rgb(215, 215, 215);border-top: 1px solid rgb(215, 215, 215);"
            >{{ item.price }}</td>
            <td
              style="border-right: 1px solid rgb(215, 215, 215);border-top: 1px solid rgb(215, 215, 215);"
            >{{ item.quantity }}</td>
            <td style="border-top: 1px solid rgb(215, 215, 215);">{{ item.description }}</td>
          </tr>
        </table>
      </div>
    </div>
  </div>
</template>
<script>
import QRCode from "qrcode";
export default {
  data() {
    return {
      data: {
        company: {},
        items: []
      }
    };
  },
  created() {
    let url = `orders/sales/quoted_price/print/${this.$route.params.id}`;
    this.$get(url)
      .then(response => {
        if (response.status != 200) return false;
        console.log(response.data)
        this.data = response.data;
        setTimeout(
          () =>
            QRCode.toString(
              `https://www.factoryun.com/print.html#/saleOffer/${this.$route.params.id}`,
              (err, string) =>
                (document.getElementById(`QrCode`).innerHTML = string)
            ),
          500
        );
      })
      .catch(err => console.error(err));
  }
};
</script>
<style lang="less" scoped>
#apply {
}
</style>
