    ...
    >
      ...
      <div style="margin-top: 15px;font-size: 10px;line-height: 2;">
        <p style="display: flex;justify-content: space-between;">
          ...
          <span style="width: 50%;">质检日期：{{ data.created_at }}</span>
          <span style="width: 50%;">质检员：{{ data.created_by }}</span>
        </p>
        <p style="display: flex;justify-content: space-between;">
          <span style="width: 50%;">暂收来源：{{ data.temp_storage.source }}</span>
          <span style="width: 50%;">单号：{{ data.quality.number }}</span>
          <span style="width: 50%;">采购专员：{{ data.quality.created_by }}</span>
        </p>
        <table
...
export default {
  ...
  created() {
    let url = `icm_qty_ctrl/stock_out_filing/print/${this.$route.params.id}`;
    this.$get(url)
      .then(response => {
        ...
        setTimeout(
          () =>
            QRCode.toString(
              `https://www.factoryun.com/print.html#/IQCchange/${this.$route.params.id}`,
              (err, string) =>
